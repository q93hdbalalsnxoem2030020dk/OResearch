...
# Initialize colorama with convert=True to properly handle color codes
...
class OResearch:
    ...
    def type(self, text: str, speed: float = 0.01):
        ...
        # Strip color codes for timing calculation but preserve them for output
        ...
        for char in text:
            ...
            # Only sleep for visible characters, not for color codes
            if char not in "\033[":
                time.sleep(speed)
        print()

    def gen_ctx(self, text: str) -> str:
        """Extract meaningful context from user input using improved NLP"""
        ...
        for item in all_context:
            if item and item.lower() not in seen:  # Check for empty strings
    ...
    def add_hist(self, query: str, response: str, source: str):
        ...
        # Clean response of color codes for storage
    ...
    def run(self):
        ...
        try:
            while True:
                ...
                # Handle search based on connectivity
                print(f"{Fore.YELLOW}[ore] Processing query...")
                
                if self.offline:
                    # Offline search using knowledge base
                    ...
                    # Online search using multiple sources
                    self.stats["online"] += 1
            ...
            print(f"{Fore.YELLOW}[ore] Please report this issue. Attempting to save data...")
            self.save_user()
