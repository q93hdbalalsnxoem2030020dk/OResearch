...
class OResearch:
        ...
        try:
            if has_gpu:
                ...
                try:
                    ...
                    print(f"{Fore.GREEN}[ore] Intent classifier loaded {Fore.CYAN}(GPU accelerated)")
                except Exception as e:
                    print(f"{Fore.YELLOW}[ore] Intent classifier not loaded: {e}")
            else:
    ...
    def hello(self):
        ...
        print(f"{Fore.CYAN}     | |     | |")
        print(f"{Fore.CYAN}     | |     | |")
        print(f"{Style.BRIGHT}{Fore.WHITE}🔍 {Fore.CYAN}OResearch v3.0{Fore.WHITE} - Enhanced AI Research Assistant")
    ...
    def type(self, text: str, speed: float = 0.01):
        ...
        for color_code in [f"{Fore.RED}", f"{Fore.GREEN}", f"{Fore.YELLOW}", 
                          f"{Fore.BLUE}", f"{Fore.MAGENTA}", f"{Fore.CYAN}", 
                          f"{Fore.WHITE}", f"{Style.BRIGHT}", f"{Style.RESET_ALL}"]:
    ...
    def detect_intent(self, text: str) -> Dict[str, float]:
        ...
        doc = nlp(text_lower)
        if any(token.tag_ in ["WP", "WRB", "WDT"] for token in doc):  # WH-question words
    ...
    def extract_entities(self, text: str) -> List[str]:
        ...
        seen = set()
        return [x for x in all_features if not (x.lower() in seen or seen.add(x.lower()))]

    def gen_ctx(self, text: str) -> Dict[str, Union[str, List[str], Dict[str, float]]]:
    ...
    def kb_search(self, context: Dict) -> str:
        ...
        if not context:
            return f"{Fore.YELLOW}I need more specific information to search my knowledge base."
        ...
        if context["is_tutorial"]:
            ...
            if tutorial_topic:
                ...
                for topic, entry in self.kb.items():
                    if tutorial_topic.lower() in topic.lower() or any(tutorial_topic.lower() in q.lower() for q in entry.get("question", [])):
                        response = random.choice(entry.get("responses", ["No tutorial available."]))
                ...
                for topic, entry in self.kb.items():
                    for q in entry.get("question", []):
                        common_words = set(tutorial_topic.lower().split()) & set(q.lower().split())
                        if common_words and len(common_words) >= 1:
                            response = random.choice(entry.get("responses", ["No tutorial available."]))
                            return self.format_tutorial(tutorial_topic, response)
                            
                return f"{Fore.YELLOW}I don't have a tutorial on '{tutorial_topic}' in my knowledge base."
                
        # Handle direct matches
        for keyword in context["keywords"]:
            if keyword.lower() in self.kb:
                entry = self.kb[keyword.lower()]
                return random.choice(entry.get("responses", ["No information available."]))
        ...
        for topic, entry in self.kb.items():
            ...
            for q in questions:
                ...
                score = overlap / max(len(query_tokens), len(q_tokens)) if max(len(query_tokens), len(q_tokens)) > 0 else 0
        ...
        if best_match and best_score > 0.3:
            ...
            if context["is_tutorial"]:
                return self.format_tutorial(context["tutorial_topic"], response)
            return response
        
        if context["is_tutorial"]:
            return f"{Fore.YELLOW}Sorry, I don't have a tutorial on that topic in my knowledge base."
        return f"{Fore.YELLOW}Sorry, I couldn't find relevant information in my knowledge base."

    def format_tutorial(self, topic: str, content: str) -> str:
        """Format content as a step-by-step tutorial"""
        if not topic or not content:
            return f"{Fore.YELLOW}I don't have enough information to create a tutorial."
        ...
        if len(sentences) >= 3:
            ...
            step_markers = ["first", "then", "next", "after", "finally", "lastly", "begin", "start"]
            
            current_step = ""
            for sentence in sentences:
                if any(marker in sentence.lower() for marker in step_markers) and current_step:
    ...
    def net_search(self, query: str, context: Dict) -> Dict:
        ...
        def duck_search():
            return {"content": self.ddg_search(search_query), "source": "DuckDuckGo"}
            ...
            while True:
                ...
                if self.offline:
                    ...
                    if results["content"]:
                        ...
                        self.type(f"{Fore.WHITE}{results['content']}\n{Fore.GREEN}Source: {results['source']}", 
                        ...
                        print(f"{Fore.YELLOW}[ore] Online search failed. Trying knowledge base...")
                        response = self.kb_search(context)
                        print(f"{Fore.CYAN}OResearch: ", end="")
                        self.type(response, speed=self.user["prefs"]["type_speed"])
